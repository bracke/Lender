...
package Controller_ListDisplay is
   ...
   type TEL_Today_Type                     is new Toolbox_UserEventListener(16#49#,-1,-1) with null record;
   ...
   --
   -- The user has clicked on the iconbar icon - open the main window.
   --
   procedure Handle (The : in TEL_OpenWindow);

   --
   -- The user has clicked on a 'today' button - find the current date and insert it.
   --
   procedure Handle (The : in TEL_Today_Type);
