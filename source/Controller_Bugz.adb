...
package body Controller_Bugz is
   ...
   Bugz_Path : constant String := "<LenderRes$Dir>.Bugz";
   ...
   begin
      if Query_Type = File_Request then
         ...
         UserMessages.SendMessage(Message    => Message'Address,
   ...
   begin
      ...
      if not Ready then
         Path := U("<Bugz$Dir>.!Run");
         Ready := Variable.Get_Value("Bugz$Dir")'Length > 0;
      end if;
      if not Ready then
         Path := U("<Boot$Dir>.^.Apps.!Bugz");
         Ready := FileExternal.Exists("<Boot$Dir>.^.Apps.!Bugz.!Run");
      end if;
      if not Ready then
         Path := U("<Boot$Dir>.Resources.!Bugz");
         Ready := FileExternal.Exists("<Boot$Dir>.Resources.!Bugz.!Run");
      end if;
      if not Ready then
         Path := U("<Boot$Dir>.^.Utilities.!Bugz");
         Ready := FileExternal.Exists("<Boot$Dir>.^.Utilities.!Bugz.!Run");
      end if;
      if not Ready then
         Path := U("<Boot$Dir>.^.DevTools.!Bugz");
         Ready := FileExternal.Exists("<Boot$Dir>.^.DevTools.!Bugz.!Run");
      end if;
      if not Ready then
         Path := U("<Boot$Dir>.^.Internet.!Bugz");
         Ready := FileExternal.Exists("<Boot$Dir>.^.Internet.!Bugz.!Run");
      end if;
